...
in this article, we will do so by the following structure:  
`varible` (*our term*) for example, `happiness score` (*score*)
...
We can see `p-value` is very high. This means we cannot reject $h_0$.  
To our suprise - The corona pandemic did not impact happiness like we thought.
...
It might be the case that when things get tough, perception of happiness increases because our day to day trouble seem less significant.  

Maybe we can isolate what in those years made people happier:
...
#### gdp and health
These are absolute scales, however it seems the variance of them drastically decreased on 2021.
Since it is of course unlikely that the population has started suddenly living more, we conclude our conversion of these variables (dividing by 100) was wrong.
...
As logic dictates, this variables are connected.  
It seems that during the unprecedented global pandemic, government `trust` level and since of `freedom` **increased**.

This is incredible - with government restrictions, limitations and conspiracy theories in mind. For some reason the citizens felt they can trust their leadership.
The `freedom` spike might be connected to suffering from lock-downs and having them removed. Similar to the  jewish tail ["the rabbi, the farmer and the sheep"](http://www.morim.com/ravez2.pdf)

This can begin to explain the mental state of the people of the different countries, leading happiness score to increase.


## 2.3 Changes in Happiness per country


In order to see improvments and declines in happiness for every country, we will introduce new variable - `diff`: diffrence in score over time
...
This is how `diff.total` per country looks like, each point is the difference single country:
...
It seems that as lower the happiness - the biggest the `diff`. pracicularly - the negative `diff`.
...
### Results
First, it is clear both that the difference for low-happiness countries is negative, while for medium and high, it is positive.  
...
We hope you enjoyed our research. We have answered many questions, and revoked others.  
