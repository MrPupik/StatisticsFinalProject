...
**$LIRI$**  
connection of gdp+freedom looks liniar, trust not linar, but is health liniat? lets check yay !!  
**$LIRI$**  
...
**$LIRI$**  
short explanation of linar regression -> goal is high r-squered
**$LIRI$**  
...
# use liniar model with geom_smooth
tidy21 %>% ggplot(aes(y=score , x=health))+
geom_point(color="lightskyblue")+ scale_y_log10()+
geom_smooth(method = "lm",color="black") + theme(text= element_text(size=20))

# liniar regression test
tidy21_lm = lm(formula =tidy21$score ~ tidy21$health ,data=tidy21)
summary(tidy21_lm)
...
**$LIRI$**  
r-sqr is 0.6 which is very sexy  what about the residuals? are they noramlly distributed? are they homogeoooo?
**$LIRI$**  

```{r residuals}


tidy21_lm %>% ggplot(aes(x=.fitted,y=.resid)) +
  geom_point(alpha=0.5, color="red") + geom_hline(yintercept=0) +
...
To comapre the happiness between the years, we started with a simple histogram - showing what scores are common for every year:
