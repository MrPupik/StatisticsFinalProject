...
# 
# IsraelData15 = data15 %>% select(`Happiness Score`,`Economy (GDP per Capita)`)
 entire_data = bind_rows(AllData[[1]],AllData[[2]],AllData[[3]], AllData[[4]], AllData[[5]], AllData[[6]], AllData[[7]])
 IsraelData = entire_data %>% filter(country == 'Israel')
#   filter(data15, Country == "Israel") %>% mutate(year=2015)

entire_data %>% ggplot(aes(x=freedom, y=score, color=as.factor(year)))  + geom_point() + scale_color_hue(h = c(150, 250))

 
entire_data %>% ggplot(aes(x=(gdp), y=score, color=as.factor(year)))  + geom_point()
AllData[[7]] %>% ggplot(aes(x=(gdp), y=score, color=as.factor(year)))  + geom_point()
 
# entire_data %>% ggplot(aes(x=`score`)) + geom_density()


View(AllData[[6]])
# ggplot(data, aes(x=))

 # entire_data %>% ggplot(aes(x=score, color=year))  + geom_histogram(bins = 30) 
